...
std::vector<GeneratedFile> McppApplicationProjectFiles(const ProjectTemplateContext& context,
  ...
  // lines; the rewrite is for the rendered version lines.
  if (manifest->content.find("\nhuxerui.huxerui = \"") == std::string::npos) {
    return files;
  }
  manifest->content = McppPathDependency(manifest->content, huxerui_home);
  // The Windows installer interface is a package of its own with its own
  // HuxerUI dependency, which names the same checkout.
  for (GeneratedFile& file : files) {
    if (file.path.generic_string() == "windows/installer/mcpp.toml") {
      file.content = McppPathDependency(file.content, huxerui_home);
    }
  }
  return files;
}
...
void CreateProject(const std::filesystem::path& destination, const ProjectTemplate& project_template,
  ...
  if (build_system == BuildSystem::Mcpp) {
    ...
    std::vector<GeneratedFile> files = McppApplicationProjectFiles(context, huxerui_home, selected);
    for (GeneratedFile& file : files) {
