...
// One declaration per resource: the generated header and the generated module
// carry the same list, so a consumer sees the same names either way.
std::map<EntryKind, std::map<std::string, std::string>> Declarations(
    std::string_view resource_namespace, const std::vector<Entry>& entries) {
  std::map<EntryKind, std::map<std::string, std::string>> declarations;
  for (const Entry& entry : entries) {
...
void WriteGroups(std::ostream& output, std::string_view resource_namespace,
                 std::map<EntryKind, std::map<std::string, std::string>>& declarations) {
  const auto write_group = [&](EntryKind kind, std::string_view name, std::string_view type) {
...
std::string GenerateHeader(std::string_view resource_namespace, const std::vector<Entry>& entries) {
  auto declarations = Declarations(resource_namespace, entries);
  std::ostringstream output;
  output << "#pragma once\n\n#include <huxerui/resource.h>\n\nnamespace " << resource_namespace << " {\n\n";
  WriteGroups(output, resource_namespace, declarations);
  output << "} // namespace " << resource_namespace << "\n";
  return output.str();
}

// The same declarations as a module interface unit. `import huxerui;` is what
// makes huxerui::RawResource and friends visible without a header, and the
// unit exports the namespace, so `import <name>;` names exactly what
// `#include "<namespace>_resources.h"` does.
std::string GenerateModule(std::string_view resource_namespace, const std::vector<Entry>& entries,
                           std::string_view module_name) {
  auto declarations = Declarations(resource_namespace, entries);
  std::ostringstream output;
  output << "// GENERATED by hrc -- do not edit by hand.\n\nexport module " << module_name
...
void Publish(
  ...
  std::filesystem::remove_all(output / "modules");
  for (const Entry& entry : entries) {
    if (!entry.package_path.empty()) {
  ...
  for (const std::string& resource_namespace : resource_namespaces) {
    ...
    if (!module_name.empty()) {
      WriteTextIfChanged(output / "modules" / (Identifier(resource_namespace) + "_resources.cppm"),
                         GenerateModule(resource_namespace, entries, module_name));
    }
  }
}
...
void Compile(const CompileOptions& options) {
  ...
  if (!options.module_name.empty()) {
    // A module name is dotted identifiers: `app.resources`, `huxerui.live2d.resources`.
    for (const char character : options.module_name) {
      const bool ok = (character >= 'A' && character <= 'Z') || (character >= 'a' && character <= 'z') ||
                      (character >= '0' && character <= '9') || character == '_' || character == '.';
      if (!ok) {
        throw std::runtime_error("resource module name must be dotted identifiers: " + options.module_name);
      }
    }
  }
  Publish(options.output, entries, {options.resource_namespace}, options.header_name, options.module_name);
