...
`HUXERUI_SMOKE_EXIT_MS=<milliseconds>` makes `RunApplication()` exit the process with status 0 after the delay, printing `HuxerUI smoke exit`; it exists so a CI step can start a GUI application and observe an exit status, and it is read by every desktop and iOS build.
