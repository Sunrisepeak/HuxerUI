...
ios.plist.LSRequiresIPhoneOS                 dist-apple states it true, and a project's Info.plist can change the value but not remove the key; CMake's template states none, and an iOS application runs on iOS either way
